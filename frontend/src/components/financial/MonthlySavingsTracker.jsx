...
/**
 * Monthly Savings Tracker Component
 * ✅ FIXED: Calculates and displays savings: Income - Budget - EMI
 */
export const MonthlySavingsTracker = ({
  monthlyIncome = 0,
  totalEMI = 0,
  totalBudget = 0,  // ✅ CHANGED from totalExpenses
  currency = 'USD',
  isLoading = false,
}) => {
  ...
  useEffect(() => {
    if (monthlyIncome > 0) {
      // ✅ FIXED: Savings = Income - Budget - EMI
      ...
      setMetrics({
        ...
        emiPercentage: Math.min(100, emiPercentage),
        incomePercentage: 100,
      })
    }
  ...
  if (isLoading) {
    return (
      <Card className="flex items-center justify-center" style={{ padding: 'var(--spacing-md)', minHeight: 'var(--placeholder-height)' }}>
        <LoadingSpinner size="lg" />
      </Card>
    )
  }

  // Status colors based on savings rate
  const getSavingsStatus = () => {
    // return both tailwind-compatible bg class (for subtle bg) and explicit colors for text/borders
    if (metrics.savingsRate >= 30) return {
      textColor: '#16a34a', // green-600
      borderColor: '#16a34a',
      borderLightColor: '#bbf7d0', // green-200-ish (approx)
      bgClass: 'bg-green-50',
      label: 'Excellent',
    }
    if (metrics.savingsRate >= 20) return {
    ...
    if (metrics.savingsRate >= 10) return {
      textColor: '#d97706', // yellow-600 / amber
      borderColor: '#d97706',
  ...
  return (
    ...
    >
      {/* Main Card */}
      <Card className={`border-l-4 ${status.bgClass}`} style={{ padding: 'var(--spacing-md)', borderLeftColor: status.borderColor }}>
            ...
            <div className="flex items-center justify-between gap-4" style={{ flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)' }}>
                <div style={{
                  ...
                  <span style={{ fontSize: 'var(--font-sm)', fontWeight: 700, color: status.borderColor }}>
            ...
            <div className="space-y-1">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full" style={{ height: 'var(--progress-height)' }}>
                <div
                  ...
                  style={{ width: '100%', height: 'var(--progress-height)' }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full" style={{ height: 'var(--progress-height)' }}>
                <div
                  ...
                  style={{ width: `${Math.min(100, metrics.emiPercentage)}%`, height: 'var(--progress-height)' }}
                />
              </div>
            ...
            <div className="space-y-1">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full" style={{ height: 'var(--progress-height)' }}>
                <div
                  ...
                  style={{ width: `${Math.min(100, metrics.budgetPercentage)}%`, height: 'var(--progress-height)' }}
                />
              </div>
            ...
            <div className="space-y-1 pt-2 border-t border-gray-300 dark:border-gray-600">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full" style={{ height: 'var(--progress-height)' }}>
                <div
                  ...
                  style={{ width: `${Math.min(100, metrics.savingsRate)}%`, height: 'var(--progress-height)', backgroundColor: status.textColor }}
                />
              </div>
      ...
      {/* Info Card: data-driven, explicit formula and values; matches dashboard styling and dark mode */}
      <Card className="bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-800" style={{ padding: 'var(--spacing-sm)' }}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-3 text-xs text-gray-600 dark:text-gray-300">
              If Available to Save is negative, your total expenses (EMI + Budget) exceed your income. Reduce budgeted expenses, refinance loans, or increase income to improve savings.
            </p>
          </div>
        </div>
      </Card>
