...
interface IncomeManagerProps {
  ...
  currency?: string
  onSave?: (payload: { monthly_salary: number }) => Promise<void> | void
  isLoading?: boolean
}
...
export const IncomeManager: React.FC<IncomeManagerProps> = ({
  ...
  const [income, setIncome] = useState<number>(initialIncome || 0)
  ...
  const getDisplayIncome = () => (annualMode ? income * 12 : income)
  const getActualMonthlyIncome = () => (annualMode ? income / 12 : income)
  ...
  const handleSave = async () => {
    if (onSave) {
      setIsSaving(true)
      try {
        await onSave({ monthly_salary: getActualMonthlyIncome() })
      } catch (error) {
      ...
      <Card className="p-6 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 border border-green-200 dark:border-green-800">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-3 bg-green-100 dark:bg-green-800/50 rounded-lg">
            <DollarSign className="icon-md text-green-600 dark:text-green-400" />
          </div>
          <div>
        ...
        <div className="mb-6 p-4 bg-white dark:bg-gray-800 rounded-lg border border-green-200 dark:border-green-700">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{annualMode ? 'Annual Income' : 'Monthly Income'}</p>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
            {annualMode ? `Monthly: ${formatCurrency(getActualMonthlyIncome(), currency)}` : `Annual: ${formatCurrency(income * 12, currency)}`}
          </p>
        </div>

        <div className="space-y-4 mb-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{annualMode ? 'Annual Income' : 'Monthly Income'}</label>
            <div className="flex gap-2">
              <Input type="number" placeholder={annualMode ? '600000' : '50000'} value={income} onChange={handleIncomeChange} step="1000" min="0" className="flex-1" />
              <Button variant="secondary" onClick={handleToggleMode} className="px-4">
          ...
          <Alert variant="info" className="mb-0">
            Income will be used to calculate your available savings each month. All calculations use the monthly amount internally.
          </Alert>
        </div>

        <div className="flex gap-3">
          <Button onClick={handleSave} isLoading={isSaving || isLoading} disabled={income === initialIncome} className="flex-1">
