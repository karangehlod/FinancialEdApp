/**
 * OAuthCallbackPage — strict TypeScript OAuth callback handler.
 *
 * Handles redirects from Google / Apple after consent (P2-6).
 * Mount at /auth/callback.
 *
 * Responsibilities (SRP):
 *  - Read OAuth params from URL
 *  - Exchange code for tokens via useOAuth hook
...
const LoadingView: React.FC = () => (
  <div
    className="flex min-h-screen flex-col items-center justify-center gap-4 bg-gray-50 dark:bg-gray-900"
    role="status"
    aria-live="polite"
    aria-label="Completing sign-in"
  >
    <div
      style={{ width: 'var(--spinner-md)', height: 'var(--spinner-md)' }}
...
const SuccessView: React.FC = () => (
  ...
  >
    <div
      ...
      style={{ width: 'var(--spinner-md)', height: 'var(--spinner-md)', backgroundColor: 'var(--success-bg, #ecfdf5)' }}
    >
      <svg
    ...
    <p className="text-sm font-medium text-gray-700 dark:text-gray-200">Signed in! Redirecting…</p>
  </div>
)
...
const ErrorView: React.FC<ErrorViewProps> = ({ message, onBack }) => (
  <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-gray-50 p-6 dark:bg-gray-900">
    <div className="w-full" style={{ maxWidth: 'var(--content-max-width)' }}>
      <div className="rounded-xl bg-white p-8 shadow dark:bg-gray-800" role="alert">
        <div className="mb-4 flex items-center gap-3">
          <div
            className="flex items-center justify-center rounded-full"
            style={{ width: 'var(--icon-md)', height: 'var(--icon-md)', backgroundColor: 'var(--danger-bg, #fff1f2)' }}
          >
            <svg
              style={{ width: 'calc(var(--icon-md) * 0.75)', height: 'calc(var(--icon-md) * 0.75)' }}
...
const OAuthCallbackPage: React.FC = () => {
  ...
  const handleSuccess = (tokens: OAuthTokens): void => {
    tokenManager.storeToken(tokens.access_token, tokens.refresh_token)
    useAuthStore.getState().fetchCurrentUser().catch(console.error)
    setStatus('success')
  ...
  useEffect(() => {
    ...
    const provider: OAuthProvider = rawProvider === 'apple' ? 'apple' : 'google'
  ...
  return <ErrorView message={errorMsg} onBack={() => navigate('/login', { replace: true })} />
}
